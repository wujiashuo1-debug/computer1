(() => {
  ...
  const KEY = 'computer_network_quiz_v1';
  const OLD_V2 = 'computer_network_unused_v2';
  const OLD_V1_PROGRESS = 'computer_network_unused_progress';
  const OLD_V1_MISTAKES = 'computer_network_unused_mistakes';

  const TYPE_LABEL = { single: '单选', multiple: '多选', truefalse: '判断', term: '名词解释', fill: '填空/计算', essay: '应用大题' };
  ...
  function renderDesktopCurrent() {
    ...
    scheduleMathTypeset($('#desktopShell'));
  }
  ...
  function renderMobileCurrent() {
    ...
    updateMobileActionBars();
    scheduleMathTypeset($('#mobileShell'));
  }
  ...
  function desktopQuestionHTML(q, reveal) {
    ...
    return `<article class="desktop-question-card" data-card-qid="${q.id}">
      ...
      <h4>${rich(q.question)}</h4>
      ${imageHTML(q)}
      ${staticOptionsHTML(q, false)}
      <div class="answer-row ${reveal ? '' : 'hidden'}"><div><strong>答案：</strong>${fmt(answerText(q))}</div>${explanationHTML(q)}</div>
  ...
  function handleDesktopCardClick(e) {
    ...
    if (answer) {
      const row = answer.closest('.desktop-question-card').querySelector('.answer-row');
      row.classList.toggle('hidden');
      scheduleMathTypeset(row);
    }
  ...
  function renderDesktopPracticePanel() {
    ...
    if (p.submitted) {
      ...
      $('#dPracticeFeedback').innerHTML = `<div>${ok ? '✅ 正确。' : '❌ 错误。'} 正确答案：${fmt(answerText(q))}</div>${explanationHTML(q)}`;
    }
    ...
    scheduleMathTypeset($('#dPracticeBox'));
  }
  ...
  function renderMobileChapters() {
    ...
    if (term) {
      ...
      if (!matches.length) return root.innerHTML = `<div class="mobile-empty">没搜到。换个关键词，例如“ARP”“拥塞控制”“DNS”。</div>`;
      root.innerHTML = matches.map(q => `<article class="mobile-chapter-card mobile-search-result">
        ...
        <h3>${rich(q.question)}</h3>
        <p>答案：${fmt(answerText(q))}</p>${explanationHTML(q)}
  ...
  function mobileQuestionHTML(q, reveal) {
    ...
    return `<div class="mobile-q-top"><span class="tag">${q.id}</span>${typeTag(q)}<span class="pill">原题 ${q.number}</span>${r.mastered ? '<span class="pill">已掌握</span>' : ''}</div>
      <h2 class="mobile-q-title">${rich(q.question)}</h2>
      ${imageHTML(q)}
      ${staticOptionsHTML(q, true)}
      ${reveal ? `<div class="mobile-answer-box"><div><strong>答案：</strong>${fmt(answerText(q))}</div>${explanationHTML(q)}</div>` : ''}
  ...
  function renderMobileQuizArea() {
    ...
    if (qz.submitted) {
      ...
      $('#mQuizFeedback').innerHTML = `<div>${ok ? '✅ 正确。' : '❌ 错误。'}<br>正确答案：${fmt(answerText(q))}</div>${explanationHTML(q)}`;
  ...
  function renderMobileMine() {
    ...
    $('#mMistakeList').innerHTML = mistakes.length ? mistakes.slice(0, 20).map(q => `<article class="mobile-mini-card">
      ...
      <h3>${rich(q.question)}</h3><p>答案：${fmt(answerText(q))}</p>${explanationHTML(q)}
  ...
  function staticOptionsHTML(q, mobileMode) {
    if (!q.options || !q.options.length) return '<div class="subjective-note">主观题/名词解释：先自己口述或在纸上写答案，再点“查看/隐藏答案”对照解析。</div>';
    const cls = mobileMode ? 'mobile-options' : 'options';
    const item = mobileMode ? 'mobile-option' : 'option';
    return `<div class="${cls}">${q.options.map(o => `<div class="${item}"><span class="key">${esc(o.key)}</span><span>${rich(o.text)}</span></div>`).join('')}</div>`;
  }

  function imageHTML(q) {
    if (!q.images || !q.images.length) return '';
    return `<div class="question-images">${q.images.map(src => `<img src="${attr(src)}" alt="题目配图" loading="lazy" />`).join('')}</div>`;
  }

  function rich(text) {
    let s = esc(String(text || ''));
    // 对常见纯文本公式做一点轻量转写，避免 2^(32-28) 这种内容继续裸奔。
    s = s.replace(/2\^\(([^)]+)\)/g, '$2^{$1}$');
    s = s.replace(/2\^(\d+)/g, '$2^{$1}$');
  ...
  function practiceQuestionHTML(q, selected, submitted, mobileMode) {
    ...
    const options = q.options.map(o => {
      ...
      return `<button class="${cls}" ${attrName}="${esc(o.key)}"><span class="key">${esc(o.key)}</span><span>${rich(o.text)}</span></button>`;
    }).join('');
    if (!mobileMode) {
      setTimeout(() => {
    ...
    return `<div class="${topClass}"><span class="tag">${q.id}</span>${typeTag(q)}<span class="pill">原题 ${q.number}</span></div><h4 class="${titleClass}">${rich(q.question)}</h4>${imageHTML(q)}<div class="${rootClass}">${options}</div>${submitted ? explanationHTML(q) : ''}`;
  }
  ...
  function buildPracticeList(scope, count, order) {
    ...
    list = list.filter(q => (q.options || []).length);
  ...
  function searchable(q) {
    return [q.id, q.chapter, q.question, q.answer, ...(q.options || []).map(o => `${o.key} ${o.text}`), ...(q.images || [])].join(' ').toLowerCase();
  }
  ...
  function explanationHTML(q) {
    return q.explanation ? `<div class="explanation-box"><strong>解析：</strong>${fmt(q.explanation)}</div>` : '';
  }

  function answerText(q) {
    if (q.type === 'truefalse') return q.answer === 'T' ? '正确 / True' : '错误 / False';
    if (!q.options || !q.options.length) return q.answer || '';
    return String(q.answer).split('').map(k => {
  ...
  function exportData(output) {
    const payload = JSON.stringify({ exportedAt: new Date().toISOString(), app: 'computer-network-quiz-v1', data }, null, 2);
    if (output) output.value = payload;
    const blob = new Blob([payload], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `computer-network-quiz-progress-${new Date().toISOString().slice(0,10)}.json`;
